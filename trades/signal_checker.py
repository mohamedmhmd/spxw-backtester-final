...
class OptimizedSignalChecker:
    ...
    def long_strangle_2_check_entry_signals(self, current_idx: int, strategy: StrategyConfig) -> bool:
        """Check strangle 1 entry signals - ULTRA OPTIMIZED VERSION"""
        # Condition 1: Volume check
        volume_threshold = self.spy_volume[0] * strategy.ls_2_volume_threshold
        vol_start = max(0, current_idx - strategy.ls_2_consecutive_candles)
        vol_end = min(self.len_spy, current_idx)
        ...
        # Condition 2: Direction check
        dir_start = max(0, current_idx - strategy.ls_2_lookback_candles)
        dir_end = min(self.len_spx, current_idx)
        ...
        # Condition 3: Range check
        range_start = max(0, current_idx - strategy.ls_2_avg_range_candles)
        range_end = min(self.len_spx, current_idx)
        ...
        avg_recent_range = np.mean(self.all_ranges[range_start:range_end])
        avg_day_range = np.mean(self.all_ranges[:current_idx])
        range_threshold = avg_day_range * strategy.ls_2_range_threshold
